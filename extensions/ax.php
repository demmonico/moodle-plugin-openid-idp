...
function trust_request_form_response_ax($request, $response, $data, $user) {
    ...
    $fields = isset($args['required']) ? explode(',', $args['required']) : array();
    $fields = ax_attr_to_uri($args, $fields);
    $fields = filter_ax_uris($fields);
    foreach ($fields as $uri) {
        ...
        if ($value !== null) {
            // ax_response setValues doesn't work because it gets the values
            // array by reference, which gets clobbered by later values.  Even
            // doing a clone doesn't work properly.  So loop over the values,
            // and add them one-by-one.
            foreach ($value as $val) {
                $ax_response->addValue($uri, $val);
            }
        }
    }

    $fields = isset($args['if_available']) ? explode(',', $args['if_available']) : array();
    $fields = ax_attr_to_uri($args, $fields);
    $fields = filter_ax_uris($fields);
    foreach ($fields as $uri) {
        $shortname = $OPENID_AX_URI_MAP[$uri];
