...
if (!defined('Auth_OpenID_VERSION')) {
    // add our local copy of the OpenID library if it can't be found in the
    // include path
    set_include_path(get_include_path() . PATH_SEPARATOR . $CFG->dirroot . '/local/openid_idp/openid-php/');
}
...
require_once($CFG->dirroot . '/local/openid_idp/extensions/ax.php');
...
class openid_helper {
    ...
    public function do_request() {
        ...
        header('X-XRDS-Location: ' . $this->make_url(array('action' => 'idpXrds'))->out(false));
    ...
    public function do_user() {
        ...
        header('X-XRDS-Location: ' . $this->make_url(array('action' => 'userXrds', 'id' => $userid))->out(false));
...
<xrds:XRDS
  ...
  <XRD>
    <Service priority=\"0\">
      ...
      <Type>$openid11</Type>
      <URI>{$this->make_url()}</URI>
      <LocalID>{$this->make_url(array('action' => 'user', 'id' => $userid))}</LocalID>
    </Service>
  </XRD>
</xrds:XRDS>";
    }
